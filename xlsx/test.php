...
foreach ($p as $q) {
      ...
      $spreadsheet->getActiveSheet()->setCellValue($cell, $q['fam_price']);
        ...
        $spreadsheet->getActiveSheet()->setCellValue('X1', "Dental Plan (Blank if none)");
        ...
        $spreadsheet->getActiveSheet()->setCellValue('Y1', "Dental Coverage (EE,ES,EC,FAM)");
		...
		$spreadsheet->getActiveSheet()->getStyle('Y1')->getFill()->getStartColor()->setARGB('FFFFCC66');
        $spreadsheet->getActiveSheet()->setCellValue('Z1', "Vision Plan");
        ...
        $spreadsheet->getActiveSheet()->setCellValue('AA1', "Vision Coverage");
...
*/
	...
	$spreadsheet->getActiveSheet()->getStyle('K2')->getAlignment()->setWrapText(true);
        ...
        $sql="select distinct employee_code, employee_id, eff_dt, term_dt, last_name, first_name from nua_monthly_member_census where company_id = " . $company_id . " and month_id = '2022-04' and dependent_code = '' order by last_name, first_name";
        $d=$X->sql($sql);
        $row=2;
        foreach($d as $e) {
            $sql="select * from nua_employee where id = " . $e['employee_id'];
